...
class Visualizer:
    ...
    def draw_dataset_dict(self, dic):
        ...
        if annos:
            ...
            labels = [x["category_id"] for x in annos]  #[1]
            colors = None
            if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get("thing_colors"):#bujin
                colors = [
                    self._jitter([x / 255 for x in self.metadata.thing_colors[c]]) for c in labels
                ]
            names = self.metadata.get("thing_classes", None)    #['Tumor', 'RectumWall']
            if names:
                labels = [names[i] for i in labels] #['RectumWall']
            labels = [
    ...
    def overlay_instances(
        ...
        *,
        ...
        if assigned_colors is None:
            ...
            if classes is None:
                for i in range(len(labels)):
                    if labels[i] == 'RectalWall|crowd':
                        assigned_colors.append(np.array([0, 1, 0]))
                    elif labels[i] == 'Tumor|crowd':
                        assigned_colors.append(np.array([1, 0, 0]))
            else:
                classes_ls = classes.numpy().tolist()
                for i in range(len(classes_ls)):
                    if classes_ls[i] == 1:
                        assigned_colors.append(np.array([0, 1, 0]))
                    elif classes_ls[i] == 0:
                        assigned_colors.append(np.array([1, 0, 0]))
        ...
        if areas is not None:
            ...
            labels = [labels[k] for k in sorted_idxs] if labels is not None else None   #['RectumWall']
