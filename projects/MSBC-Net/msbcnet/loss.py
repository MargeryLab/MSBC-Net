...
"""
MSBC-Net model and criterion classes.
"""
import torch
...
class SetCriterion(nn.Module):
    """ This class computes the loss for MSBC-Net.
    ...
    def __init__(self, cfg, num_classes, matcher, weight_dict, eos_coef, losses, use_focal):
        ...
        if self.use_focal:
            self.focal_loss_alpha = cfg.MODEL.MSBCNet.ALPHA
            self.focal_loss_gamma = cfg.MODEL.MSBCNet.GAMMA
        else:
    ...
    def forward(self, outputs, targets):
        ...
        if self.mask_on:
            losses.update(outputs['bmask_loss'])
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                if self.mask_on:
                    mask_loss = aux_outputs['bmask_loss']
                    mask_loss = {k + f'_{i}': v for k, v in mask_loss.items()}
